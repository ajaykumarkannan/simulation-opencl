...
MonteCarloAsian::setupCL(void)
{
    ...
    kernelFile.open("MonteCarloAsian_Kernels.cl");
...
{
    std::string strArray[3] = {"Steps", "Time(sec)", "Samples used /sec"};
    std::string stats[3];
        
    totalTime = setupTime + kernelTime;
    stats[0] = sampleCommon->toString(steps, std::dec);
    stats[1] = sampleCommon->toString(totalTime, std::dec);
    stats[2] = sampleCommon->toString((noOfTraj * (noOfSum - 1) * steps) / totalTime, std::dec);
    
    this->SDKSample::printStats(strArray, stats, 3);
}

void
MonteCarloAsian::lshift128(unsigned int* input, unsigned int shift, unsigned int * output)
{
...
void
MonteCarloAsian::rshift128(unsigned int* input, unsigned int shift, unsigned int* output)
{
    unsigned int invshift = 32u - shift;
