...
BlackScholes::setupCL(void)
{
    ...
    kernelFile.open("BlackScholes_Kernels.cl");
...
BlackScholes::runCLKernels(void)
{
     ...
     * So, the number of  threads (global) is half the width.
     */
...
int BlackScholes::run()
{
	...
	if(!quiet)
    {
        sampleCommon->printArray<cl_float>("deviceCallPrice", deviceCallPrice, width, 1);
        sampleCommon->printArray<cl_float>("devicePutPrice", devicePutPrice, width, 1);
    }
...
int BlackScholes::verifyResults()
{
    if(verify)
    {
        ...
        if(compareArray(deviceCallPrice, hostCallPrice, width * height * 4))
        {
            std::cout<<"Failed\n";
            return SDK_FAILURE;
        }
        ...
        if(compareArray(devicePutPrice, hostPutPrice, width * height * 4))
        {
            std::cout<<"Failed\n";
            return SDK_FAILURE;
        }
...
void BlackScholes::printStats()
{
   ...
   totalTime = setupTime + kernelTime;
    ...
    std::string strArray[3] = {"Option Samples", "Time(sec)", "Options/sec"};

    std::string stats[3];
    stats[0] = sampleCommon->toString(actualSamples          , std::dec);
    stats[1] = sampleCommon->toString(totalTime              , std::dec);
    stats[2] = sampleCommon->toString(actualSamples/totalTime, std::dec);
