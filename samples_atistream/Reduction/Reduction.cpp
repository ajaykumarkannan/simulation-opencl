...
Reduction::setupReduction()
{
    ...
    if(!quiet) 
    {
        sampleCommon->printArray<cl_uint>("Input", input, length * VECTOR_SIZE, 1);
    }
...
Reduction::setupCL()
{
    ...
    kernelFile.open("Reduction_Kernels.cl");
...
Reduction::runCLKernels()
{
    ...
    int numBlocks = length / GROUP_SIZE;
