...
FloydWarshall::setupCL(void)
{
    ...
    pathDistanceBuffer = clCreateBuffer(
                        context, 
    ...
    kernelFile.open("FloydWarshall_Kernels.cl");
    ...
    for(cl_uint i = 0; i < numPasses; ++i)
    {
        ...
        if(timing)
        {
            ...
            * we store the kernel start time   */
            ...
            totalKernelTime += (cl_double)(kernelsEndTime - kernelsStartTime)/1e9;
...
{
    std::string strArray[2] = {"Nodes", "Time(sec)"};
    std::string stats[2];
